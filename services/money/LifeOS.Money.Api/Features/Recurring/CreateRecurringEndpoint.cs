...
public static class CreateRecurringEndpoint
{
    ...
    // priceless plan items per ADR-0029). Idempotent on the client-assigned RecurringId
    // (ADR-0003): a repeat id is a 409, which the client treats as already-applied. Live
    // requires a rule + estimate; Materialized requires priceless Items + bare-money
    // payments, authored once here (a plan is immutable except cancel; no balance).
        ...
        IDocumentSession session)
    {
        ...
        {
            var items = RecurringMapping.ToPlanItems(request.Items ?? [], request.Currency);
