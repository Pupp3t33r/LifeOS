...
**Materialized restructure — [ADR-0028](./docs/adr/0028-recurring-contents-at-root.md) (Accepted 2026-07-01, amended in place 2026-07-02), pending implementation.** The current Materialized shape above (`ScheduleLine` carries a per-payment `list<Line>`) is superseded: line-item **contents move to a root `Items` list** on the aggregate (symmetric with Live's `EstimateLines`), and a **`ScheduleLine` becomes bare money** (`{ LineId, DueDate, Amount }`), with the invariant **Σ payments = Σ items**. A plan is *one uniformly-financed purchase*; confirm records a **server-computed proportional slice** of the items (deterministic, cumulative-exact — budgets exact), with **no amount/line override** (override stays Live-only). A plan is **authored once and immutable except cancel** — the per-line edit/remove path (`ScheduleLineAdded/Edited/Removed`) is **removed**, not reshaped; changing terms is cancel + recreate, and `RecurringPaymentCancelled` gains a **refund / no-refund** flag. `Items` are *contents*, not asset valuation. Grouping / "what's in it" / order status reuse **ADR-0022** (Wishlist items + Packages) via `Line.WishlistItemId` — not the payment plan. UI: two named create surfaces, **Ongoing** (Live) and **Payment plan** (Materialized) — see `apps/wallet/docs/design/recurring/`. Touches: aggregate fields/factory/deciders/Apply, the Materialized events (create carries `Items` + bare schedule; per-line edit events dropped; cancel gains refund flag), `RecurringOccurrences`, `ConfirmOccurrenceEndpoint`, the occurrences projection, and tests. Live mode is untouched. **Out of scope (noted in the ADR):** a future **Credit** recurring type (interest + prepayment re-amortization + initial-loan-as-income; *not* a mutation of the interest-free plan), item valuation, delivery-cost allocation, FX/inflation.
