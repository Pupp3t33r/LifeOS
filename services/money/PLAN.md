...
**Materialized restructure — [ADR-0028](./docs/adr/0028-recurring-contents-at-root.md) (Accepted 2026-07-01, amended in place 2026-07-02) — DONE (2026-07-02).** The old Materialized shape (`ScheduleLine` carried a per-payment `list<Line>`) is superseded: line-item **contents live in a root `Items` list** on the aggregate (symmetric with Live's `EstimateLines`), and a **`ScheduleLine` is bare money** (`{ LineId, DueDate, Amount }`), with the invariant **Σ payments = Σ items** enforced at create. A plan is *one uniformly-financed purchase*; confirm records a **server-computed proportional slice** of the items (`ProportionalAllocation` — cumulative largest-remainder, order-independent, budgets exact), with **no amount/line override** (a plan-payment override is a 400; override stays Live-only). A plan is **authored once and immutable except cancel** — the per-line edit/remove path (`ScheduleLineAdded/Edited/Removed` + their 3 endpoints) is **removed**; changing terms is cancel + recreate, and `RecurringPaymentCancelled` carries a **`Refunded`** flag (set via `POST /recurring/{id}/cancel?refunded=true`; the refund flow itself is deferred). `Items` are *contents*, not asset valuation. Grouping / "what's in it" / order status reuse **ADR-0022** (Wishlist items + Packages) via `Line.WishlistItemId` — not the payment plan. UI: two named create surfaces, **Ongoing** (Live) and **Payment plan** (Materialized) — see `apps/wallet/docs/design/recurring/` (Flutter build pending). **Event break (pre-prod, no migration):** `RecurringPaymentCreated` gains `Items`; `RecurringPaymentCancelled` gains `Refunded` — reset the local `money-db` volume. **Out of scope (noted in the ADR):** a future **Credit** recurring type (interest + prepayment re-amortization + initial-loan-as-income; *not* a mutation of the interest-free plan), item valuation, delivery-cost allocation, FX/inflation.
