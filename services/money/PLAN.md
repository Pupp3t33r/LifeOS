...
**Materialized restructure — [ADR-0028](./docs/adr/0028-recurring-contents-at-root.md) (Accepted 2026-07-01, amended in place 2026-07-02) — DONE (2026-07-02).** The old Materialized shape (`ScheduleLine` carried a per-payment `list<Line>`) is superseded: line-item **contents live in a root `Items` list** on the aggregate (symmetric with Live's `EstimateLines`), and a **`ScheduleLine` is bare money** (`{ LineId, DueDate, Amount }`). A plan is **authored once and immutable except cancel** — the per-line edit/remove path (`ScheduleLineAdded/Edited/Removed` + their 3 endpoints) is **removed**; changing terms is cancel + recreate, and `RecurringPaymentCancelled` carries a **`Refunded`** flag (set via `POST /recurring/{id}/cancel?refunded=true`; the refund flow itself is deferred). Grouping / "what's in it" / order status reuse **ADR-0022** (Wishlist items + Packages) via `Line.WishlistItemId` — not the payment plan. UI: two named create surfaces, **Ongoing** (Live) and **Payment plan** (Materialized) — see `apps/wallet/docs/design/recurring/`.

**Priceless plan items — [ADR-0029](./docs/adr/0029-recurring-materialized-priceless-contents.md) (Accepted 2026-07-07, supersedes ADR-0028 §3–§4) — DONE (2026-07-07).** ADR-0028's *priced* items + balance invariant + proportional-slice confirm did not match reality: all-in deals are discounted and never itemised by real per-item cost, and forcing the user to enter per-item prices that sum to the payments is tedious and fictional. So: a Materialized item becomes a **priceless `PlanItem`** (`{ Description, ReferenceValue?, CategoryId?, WishlistItemId? }`) — `ReferenceValue` is an optional informational MSRP-style figure, never validated or summed; the **balance invariant is dropped** (plan total = **Σ payments**, emergent); and **confirm records a plain reference** — a single line at the amount actually paid (default scheduled, **adjustable**) under the plan's category + the plan back-ref, **no itemisation, no slice**. `ProportionalAllocation` + `SliceForOccurrence` deleted. Per-item cost is a **deferred Phase-3 net-worth valuation** (an MSRP-weighted split of the actual total; `ReferenceValue` is stored now purely as its input). Retains ADR-0028 §2 (money-only payment) and §5 (grouping via ADR-0022). Flutter (create sheet + plan resolve sheet) rebuilt to match; analyze green; 143 money tests green. **Event break (pre-prod, no migration):** `RecurringPaymentCreated.Items` changes from `list<Line>` to `list<PlanItem>` and the balance invariant is gone — **reset the local `money-db` volume**. **Out of scope (unchanged):** a future **Credit** recurring type, item valuation / delivery-cost allocation / FX / inflation.
...
- **"Unconfirmed only" guard on Materialized line edit/remove** — ADR-0017 forbids editing/removing a line whose occurrence is already confirmed on a period; not enforced (needs a write-time period join). Currently unguarded. **Dissolved by ADR-0028:** that restructure removes the per-line edit/remove path entirely (a plan is immutable except cancel), so once 0028 lands this gap no longer exists — there is nothing to guard.
